...
export async function requireLiveRun(c:PoolClient,tenant:string,site:string,run:string,deletions:DeletionLedger,lockRows=true){
  ...
  const r=(await c.query(`SELECT r.*,t.deletion_epoch,f.deletion_epoch AS pinned_epoch,f.quarantined FROM crawl r JOIN tenant t ON t.id=r.tenant_id
    JOIN work_fence f ON f.tenant_id=r.tenant_id AND f.crawl_id=r.id WHERE r.tenant_id=$1 AND r.site_id=$2 AND r.id=$3 ${lockRows?'FOR UPDATE OF r':''}`,[tenant,site,run])).rows[0];
...
export async function requireLease(c:PoolClient,l:Lease,deletions:DeletionLedger,lockRows=true){
  ...
  const r=await requireLiveRun(c,l.tenantId,l.siteId,l.runId,deletions,lockRows);
  const j=(await c.query(`SELECT * FROM job WHERE tenant_id=$1 AND site_id=$2 AND crawl_id=$3 AND job_id=$4 AND state='leased' AND lease_token=$5 AND lease_until>clock_timestamp() AND deadline>clock_timestamp() AND attempt=$6 ${lockRows?'FOR UPDATE':''}`,[l.tenantId,l.siteId,l.runId,l.jobId,l.token,l.attempt])).rows[0];
