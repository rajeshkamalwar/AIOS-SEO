...
implementation: ["packages/perception/collector.ts", "packages/perception/url.ts"]
tests: ["tests/collector.test.ts"]
...
scope: "Redirect target admission, origin constraints, per-hop handoff and status retention."
baseline: "2b6d527"
...
# CAP-HTTP-02 — Redirect semantics

Canonical responsibility: [CAP-HTTP-02](../../docs/14-CAPABILITY-TAXONOMY.md) (exact row ID); inherited [contract](../../docs/15-CAPABILITY-CONTRACT.md) and [archetype applicability](../../docs/18-BUSINESS-ARCHETYPE-MATRIX.md). Definition is deliberately not copied.

Full capability maturity: **DEFINED**, completion: **INCOMPLETE**. Local evidence scope: `foundation_only` / `TESTED`.

Redirect target admission, origin constraints, per-hop handoff and status retention.

Implementation: [collector.ts](../../packages/perception/collector.ts), [url.ts](../../packages/perception/url.ts)
...
Remaining: No intent-aware redirect semantic assessment or migration recommendation.
