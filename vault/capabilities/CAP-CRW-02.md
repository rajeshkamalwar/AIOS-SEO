...
implementation: ["packages/perception/index.ts", "packages/jobs/index.ts", "packages/jobs/frontier.ts"]
tests: ["tests/perception.test.ts", "tests/jobs.test.ts", "tests/frontier-persistence.test.ts"]
...
scope: "Fixed priority/depth/URL ordering plus fair leased job queue and bounded admissions."
baseline: "2b6d527"
...
# CAP-CRW-02 — Crawl prioritization

Canonical responsibility: [CAP-CRW-02](../../docs/14-CAPABILITY-TAXONOMY.md) (exact row ID); inherited [contract](../../docs/15-CAPABILITY-CONTRACT.md) and [archetype applicability](../../docs/18-BUSINESS-ARCHETYPE-MATRIX.md). Definition is deliberately not copied.

Full capability maturity: **DEFINED**, completion: **INCOMPLETE**. Local evidence scope: `partial_behavior` / `TESTED`.

Fixed priority/depth/URL ordering plus fair leased job queue and bounded admissions.

Implementation: [index.ts](../../packages/perception/index.ts), [index.ts](../../packages/jobs/index.ts), [frontier.ts](../../packages/jobs/frontier.ts)

Tests: [perception.test.ts](../../tests/perception.test.ts), [jobs.test.ts](../../tests/jobs.test.ts), [frontier-persistence.test.ts](../../tests/frontier-persistence.test.ts)
...
Remaining: No evaluated page-role/change-history refresh scheduling, representative-template policy or end-to-end public crawl allocation.
