...
def main():
 ...
 for c in caps:
  ...
  if c['completion']=='COMPLETE':
   ...
   assert all(g['status']=='CLOSED' for g in gaps if g['id'] in c['gaps'] and g['severity'] in {'CRITICAL','HIGH'}),c['id']
 ...
 paths += [ROOT/p for p in ['AGENTS.md','README.md','package.json','package-lock.json','tsconfig.json','vault/refresh.py']]
 ...
 state+='The owner-approved N0 deployment/data-use profile gates customer processing. It does not block reversible local fixes such as [submission deadline](gaps/GAP-025.md) and [graph integrity](gaps/GAP-026.md), nor grant any website-write authority.\n\n'
 ...
 cov='# Coverage matrix\n\n'+stamp+'All36domains/195canonical rows are indexed. Full-capability maturity and local prerequisite maturity are separate. **No full-capability completion is asserted.** Names are navigation labels; definitions remain in [14](../docs/14-CAPABILITY-TAXONOMY.md) and inherited contracts15/18.\n\n'
 ...
 if args.base:
  ...
  if any(p.startswith(('packages/','apps/','workers/','tests/','scripts/')) or p in {'package.json','package-lock.json','tsconfig.json','vault/refresh.py'} for p in changed):
