...
if (process.argv.slice(2).length > 1 || process.argv.slice(2).some(arg => !["--http-seed-dispatch", "--compiled-http-seed-dispatch", "--http-seed-dispatch-docker", "--http-seed", "--compiled-http-seed", "--http-seed-docker", "--http-projection", "--compiled-http-projection", "--http-projection-docker", "--http-acceptance", "--compiled-http-acceptance", "--http-acceptance-docker", "--http-bootstrap", "--compiled-http-bootstrap", "--fixture-retention", "--compiled-fixture-retention", "--http-isolated", "--compiled-http-isolated", "--http-isolated-docker", "--render-projection", "--compiled-render-projection", "--render-projection-docker", "--render-acceptance-docker", "--render-acceptance", "--compiled-render-acceptance", "--compiled-http", "--render-source", "--compiled-render-source", "--compiled-render-lane", "--render-execution", "--compiled-render-execution"].includes(arg))) throw new Error("Unknown or combined test mode");
const httpSeedDispatchDocker=process.argv.includes("--http-seed-dispatch-docker");
const httpSeedDispatch=process.argv.includes("--http-seed-dispatch")||httpSeedDispatchDocker;
const compiledHttpSeedDispatch=process.argv.includes("--compiled-http-seed-dispatch");
...
try {
  ...
  const result = spawnSync(
    ...
    compiledHttp ? ["--test", "--test-concurrency=1", "dist/tests/foundation.test.js", "dist/tests/http-lane.test.js"] : [
      ...
      "tests/http-seed.test.ts",
      "tests/http-seed-fixture-broker.test.ts",
    ...
    ],
    {
      ...
      env: {
        ...process.env,
        AIOS_TEST_HTTP_SEED_DISPATCH: httpSeedDispatchDocker ? "1" : "0",
