...
before(async()=>{
 ...
 const blobs=await LocalBlobs.create(join(process.env.AIOS_TEST_ROOT!,'http-isolated-blobs'),'local-synthetic-v1');ledger=new Ledger(runtime,blobs,'local-synthetic-v1');
...
test('Isolated broker makes one governed fixture hop and retains metadata only with unknown byte settlement',async t=>{
 ...
 const before=await counts(),result=await new IsolatedHttpFixtureSupervisor(worker,lane,supervisor,fakeEngine()).collect(lease,{fixturePort:server.port});
 validate(base+'http-bootstrap-execution.schema.json',result);assert.equal(result.reservation.state,'settled');assert.equal(result.state,'completed');assert.equal(result.evidenceAccepted,false);assert.equal(hits,1);assert.deepEqual(result.result,{status:200,truncated:false,retainedBodyBytes:body.length,retainedBodySha256:hash(body)});assert.ok(!JSON.stringify(result).includes('/private-fixture'));
 const row=await stored(origin);assert.equal(row.actual_bytes,null);assert.equal(row.reserved_bytes,'512000');assert.ok(row.settled_at);assert.equal(row.in_flight,0);assert.equal((await counts()).evidence,before.evidence);assert.equal((await counts()).snapshots,before.snapshots);
...
test('One-hop broker does not follow redirects or forward fixture headers',async t=>{
 ...
 const result=await new IsolatedHttpFixtureSupervisor(worker,lane,supervisor,fakeEngine()).collect(lease,{fixturePort:server.port});
...
test('Broker overflow is bounded and worker repeated fetch cannot allocate an uncharged second hop',async t=>{
 ...
 const server=await localFixture((_,res)=>{hits++;res.writeHead(200,{'content-type':'text/plain'});res.end('x'.repeat(600000));});t.after(server.close);
 const result=await new IsolatedHttpFixtureSupervisor(worker,lane,supervisor,fakeEngine({repeat:true})).collect(lease,{fixturePort:server.port});
 assert.equal(hits,1);assert.ok(result.result===null||result.result.truncated);if(result.result)assert.ok(result.result.retainedBodyBytes<=512000);assert.equal((await stored(origin)).reserved_bytes,'512000');
});
test('Forged worker metadata cannot become a successful broker result',async t=>{
 ...
 const result=await new IsolatedHttpFixtureSupervisor(worker,lane,supervisor,fakeEngine({forge:true})).collect(lease,{fixturePort:server.port});assert.equal(result.state,'failed');assert.equal(result.result,null);assert.equal((await stored(origin)).in_flight,0);
});
test('Current cancellation before GO and before broker socket both prevent fixture access',async t=>{
 for(const phase of ['beforeStart','beforeRequest'] as const){
  ...
  const pending=new IsolatedHttpFixtureSupervisor(worker,lane,supervisor,engine).collect(lease,{fixturePort:server.port});
...
test('Uncertain independent container cleanup cannot author terminal proof or release capacity',async t=>{
 ...
 await assert.rejects(new IsolatedHttpFixtureSupervisor(worker,lane,supervisor,fakeEngine({cleanupFails:true})).collect(lease,{fixturePort:server.port}),/container_state_uncertain|termination_unconfirmed/);
...
test('Public URLs, non-robots paths and aborted input fail before reservation or engine creation',async()=>{
 const origin='https://isolated-admission.example',r=await run(origin),lease=await r.next();let creates=0;const engine=fakeEngine(),create=engine.create;engine.create=async()=>{creates++;return create();};const producer=new IsolatedHttpFixtureSupervisor(worker,lane,supervisor,engine);
 for(const url of ['https://example.com/robots.txt',origin+'/page',origin+'/robots.txt?key=secret'])await assert.rejects(producer.collect(lease,{url,fixturePort:12345,maxDecodedBytes:1000} as any),/fixture_only|credential|query|invalid/);
 await assert.rejects(producer.collect(lease,{fixturePort:12345},{signal:AbortSignal.abort()}),/aborted/);assert.equal(creates,0);assert.equal(await stored(origin),undefined);await commands.cancel(r.p,r.site,r.id);
});
test('Abort during broker response closes the socket before accounting settlement',async t=>{
 ...
 const result=await new IsolatedHttpFixtureSupervisor(worker,lane,supervisor,fakeEngine()).collect(lease,{fixturePort:server.port},{signal:controller.signal});
...
if(process.env.AIOS_TEST_HTTP_ISOLATED==='1')test('Actual network-none HTTP container requests one host-mediated fixture hop and is independently removed',async t=>{
 ...
 const result=await new IsolatedHttpFixtureSupervisor(worker,lane,supervisor,engine).collect(lease,{fixturePort:server.port});
...
test('Release revocation after container start but before broker request prevents opening a socket',async t=>{
 ...
 const server=await localFixture((_,res)=>{hits++;res.end('must not request');});t.after(server.close);
 await assert.rejects(new IsolatedHttpFixtureSupervisor(worker,lane,supervisor,fakeEngine({beforeRequest:async()=>{await registry.change(httpDigest,'revoked','fixture');}})).collect(lease,{fixturePort:server.port}),/broker_termination_unverified/);assert.equal(hits,0);assert.equal((await stored(origin)).in_flight,1);
});
