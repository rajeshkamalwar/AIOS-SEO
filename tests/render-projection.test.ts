...
if(process.env.AIOS_TEST_RENDER_PROJECTION==='1')test('Actual governed Docker path retains private artifacts and commits typed samples with atomic job completion',async()=>{
 ...
 const engine=new DockerOfflineRenderEngine({imageDigest,browserBuild,...(process.env.AIOS_DOCKER_CONTEXT?{dockerContext:process.env.AIOS_DOCKER_CONTEXT}:{})}),f=await fixture(20,2000,reviewedRawDom),lease=await child(f,await renderRelease());
 ...
 for(const id of [result.acceptance.receiptEvidenceId,...result.acceptance.domEvidenceIds]){const row=(await admin.query('SELECT * FROM aios.evidence WHERE id=$1',[id])).rows[0],bytes=await blobs.read(row.artifact_key,row.sha256,Number(row.bytes));assert.doesNotMatch(bytes.toString(),/<(?:form|input|textarea)\b/i);}
