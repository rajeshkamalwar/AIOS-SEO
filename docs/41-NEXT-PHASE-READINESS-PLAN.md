...
Implementation status: transport safety, robots/sitemap/frontier conformance, durable perception storage, scope receipts and fixture sitemap/link frontiers, retained synthetic HTTP receipts, fenced fixture snapshot projection, persistent HTTP accounting, a loopback-to-snapshot integration and offline Linux/Chromium sandbox conformance are implemented and tested. See the [N1 implementation report](reports/N1-IMPLEMENTATION.md) for actual boundaries. Governed live dispatch with integrated per-hop/byte accounting, complete frontier recovery and governed live-render integration remain open; the local integration test does not establish production egress safety.
