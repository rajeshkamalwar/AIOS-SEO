...
Implementation status: transport safety, robots/sitemap/frontier conformance, durable perception storage and submitted seeds, retained synthetic HTTP receipts, fenced fixture snapshot projection, persistent HTTP accounting and a loopback-to-frozen-bundle integration are implemented and tested. See the [N1 implementation report](reports/N1-IMPLEMENTATION.md) for actual boundaries. Governed live dispatch with integrated per-hop/byte accounting, complete frontier recovery and the isolated renderer remain open; the local integration test does not establish production egress safety.
